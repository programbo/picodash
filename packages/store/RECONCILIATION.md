...
- Branch snapshot: `cad09e42` baseline plus the Store alpha integration/destroyScope slice
...
| `packages/store/src/runtime-controller.ts` | RETAIN   | Private root-keyed runtime controller for active provider/entity/relationship generations and descendant traversal.                                                               | Store-owned private runtime support.                                                                                   | Keep unreachable from public package exports and extend only for accepted lifecycle slices.                      |
...
## Test and support-module ledger (18 files)
...
- Store integration lease protocol is implemented and verified; binding leases, orientation overrides, and diagnostics remain unresolved later slices.
