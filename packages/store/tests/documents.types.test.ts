...
test('public Store document namespaces are conditional and preserve result types', () => {
  ...
  const identifiedImportOnly = createPicodashStore({
    valueOwner: 'store',
    storeId: 'documents-types-import-only',
    schemaVersion: 1,
    fields: { value: { defaultValue: 1 } },
  })
  ...
  const exportEnabled = createPicodashStore({
    valueOwner: 'store',
    storeId: 'documents-types-export',
    schemaVersion: 1,
    fields: { value: { defaultValue: 1 } },
  ...
  const persistentExport = createPicodashStore({
    valueOwner: 'store',
    storeId: 'documents-types-persistent',
    schemaVersion: 1,
    fields: { value: { defaultValue: 1 } },
    export: { documents: { defaultFieldPolicy: 'include' } },
    persistence: {
