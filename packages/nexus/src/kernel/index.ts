...
/** The JSON values accepted at Nexus trust boundaries. */
...
/** Issues supplied by application callbacks. Nexus-owned codes are not accepted here. */
...
type InferredNexusConfigBase<Values extends Record<string, PicodashJsonValue>> = {
...
type InferredNexusConfig<Values extends Record<string, PicodashJsonValue>> =
  | (InferredNexusConfigBase<Values> & {
  ...
  | (InferredNexusConfigBase<Values> & {
      ...
      readonly persistence?: NexusOwnedPersistenceConfig<
...
type NexusOwnedConfigBase<Fields extends Record<string, FieldLike>> = {
  readonly valueOwner: 'nexus'
  readonly adapter?: never
  readonly fields: DefinitionsFor<Fields>
...
export type NexusOwnedConfig<Fields extends Record<string, FieldLike>> =
  | (NexusOwnedConfigBase<Fields> & {
      readonly nexusId?: string
      readonly schemaVersion?: number
      readonly initialEnvelope?: never
      readonly persistence?: never
  ...
  | (NexusOwnedConfigBase<Fields> & {
      ...
      readonly schemaVersion: number
      readonly initialEnvelope?: PicodashEnvelopeInput<ValuesOf<Fields>>
      readonly persistence?: NexusOwnedPersistenceConfig<Fields>
      readonly migrations?: SchemaMigrations
      readonly export?: PicodashExportConfig
    })
...
export type RootNexusSnapshot<Values extends object> = {
  readonly values: Readonly<Values>
...
export type ScopedNexusSnapshot<Values extends object> = {
  readonly values: Readonly<Values>
...
>
  ...
  scope(scopeId: string): ScopedNexus<Fields, Result, Identified, ExportEnabled>
...
>
  ...
  readonly root: RootNexus<Fields, Result, Identified, ExportEnabled>
  readonly scopeId: string
  readonly fields: PicodashFields<Fields>
  readonly diagnostics: PicodashDiagnostics
  scope(scopeId: string): ScopedNexus<Fields, Result, Identified, ExportEnabled>
...
export type RootNexus<
  Fields extends Record<string, FieldLike>,
  Result extends CoreTransactionResult = CoreTransactionResult,
  Identified extends boolean = false,
  ExportEnabled extends boolean = false,
> = RootNexusBase<Fields, Result, Identified, ExportEnabled> &
...
export type ScopedNexus<
  Fields extends Record<string, FieldLike>,
  Result extends CoreTransactionResult = CoreTransactionResult,
  Identified extends boolean = false,
  ExportEnabled extends boolean = false,
> = ScopedNexusBase<Fields, Result, Identified, ExportEnabled> &
  PersistenceCapability<Result> &
  MetadataRecoveryCapability<Identified, Result> &
  (Identified extends true
...
const normalizeIssue = (input: unknown, fallbackCode: PicodashIssueCode): TransactionIssue => {
  ...
  const message = typeof source.message === 'string' ? source.message : 'Nexus operation failed.'
...
const normalizeIssues = (
  ...
  try {
    return Object.freeze(
      (issues?.length ? issues : [{ code: fallbackCode, message: 'Nexus operation failed.' }]).map(
...
>(
  config: InferredNexusConfig<Values> & {
...
>(
...
): RootNexus<Definitions, CoreTransactionResult, boolean, boolean> {
  type Fields = Definitions
  if (
    !config ||
    typeof config !== 'object' ||
    (config.valueOwner !== 'nexus' && config.valueOwner !== 'external')
  )
  ...
  const configuredNexusId = config.nexusId
  ...
  if (configuredExport !== undefined) {
    if (
      !validIdentity(configuredNexusId) ||
  ...
  if (configuredNexusId !== undefined && !validIdentity(configuredNexusId))
    throw new PicodashContractError('invalid-configuration')
  if (
    configuredSchemaVersion !== undefined &&
    (!validIdentity(configuredNexusId) ||
  ...
  if (
    ...
    throw new PicodashContractError('invalid-configuration')
  if (config.valueOwner === 'nexus' && config.adapter !== undefined)
    throw new PicodashContractError('invalid-configuration')
  ...
  const configuredPersistence = (
    config as {
      readonly persistence?:
        | NexusOwnedPersistenceConfig<Definitions>
        | ExternalOwnedPersistenceConfig
    }
  ).persistence
        ...
        : new Set(
            configuredFieldKeys.filter((key) => {
              const storePersistence =
                configuredPersistence as NexusOwnedPersistenceConfig<Definitions>
  ...
  if (configuredPersistence === undefined && config.initialEnvelope !== undefined) {
    const nexusId = configuredNexusId
    const schemaVersion = configuredSchemaVersion
    if (
      !validIdentity(nexusId) ||
    ...
    try {
      hydrated = hydratePersistenceEnvelope(
        ...
        { nexusId, schemaVersion, valueOwner: config.valueOwner },
        (input) => {
          if (config.valueOwner === 'external')
            return input &&
              typeof input === 'object' &&
        ...
        },
        {
          ...
          onUnknownFieldCount: (count) => {
            ...
            diagnosticsRuntime.recordCondition({
              ...
              severity: 'warning',
              message: 'Persisted fields are not present in the current Nexus schema.',
    ...
    if (config.valueOwner === 'nexus') values = hydrated.record.values
  ...
  if (configuredPersistence !== undefined) {
    try {
        ...
        onFailure: (reason: PersistenceFailureReason) => {
          if (!runtimeController)
            return Object.freeze({
              ...
              message: 'Nexus persistence failed.',
      ...
      if (config.valueOwner === 'nexus') values = persistenceController.initialValues
  ...
  let currentSnapshot!: RootNexusSnapshot<ValuesOf<Fields>>
  ...
  type ScopedChannel = {
    readonly scopeId: string
    snapshot: ScopedNexusSnapshot<ValuesOf<Fields>>
  ...
  currentSnapshot = freeze({ values, scopes }) as RootNexusSnapshot<ValuesOf<Fields>>

  type NexusResult = CoreTransactionResult
  let metadataRecoveryCapability!: PicodashMetadataRecovery<NexusResult>
  let nexus!: RootNexus<Fields, NexusResult, boolean, boolean>
  let metadataRecoverySnapshot: PicodashMetadataRecoveryState = Object.freeze({
    quarantinedScopes,
  })
  const metadataRecoveryRuntime = createMetadataRecovery<NexusResult>({
    assertActive: () => {
  ...
  const documentFailure = (
    reason: string,
    message = 'Invalid Nexus document.',
  ): Extract<CoreTransactionResult, { readonly ok: false }> =>
    (() => {
      const code: PicodashIssueCode =
        reason === 'foreign_nexus'
          ? 'foreign_nexus'
  ...
  const documentScopeIds = (): readonly string[] => {
    const controller = runtimeControllerFor(nexus as object)
  ...
  const executeDocumentExportPlan = (
    ...
    withWriteLock(() => {
      ...
      if (record.root !== (nexus as object)) documentPlanError('export', 'foreign-root')
  ...
  const makeImportSnapshot = (
    ...
    if (document.nexusId !== configuredNexusId && !options.allowForeignNexus)
      return documentFailure('foreign_nexus')
  ...
  const createDocumentImportPlanLocked = (
    ...
    const review = normalizePicodashImportPlanReview({
      ...
      foreignNexus: snapshot.document.nexusId !== configuredNexusId,
    })
  ...
  const executeDocumentImportPlanLocked = (
    ...
    if (record.root !== (nexus as object)) documentPlanError('import', 'foreign-root')
  ...
  const storeImplementation: RootNexus<Fields, NexusResult, boolean, boolean> = {
    kind: 'root',
    fields,
    ...
    resetValueOrThrow(field) {
      const result = nexus.resetValue(field)
      if (!result.ok) throw result.error
      return result
    },
    ...
    resetRegisteredValuesOrThrow(options) {
      const result = nexus.resetRegisteredValues(options)
      if (!result.ok) throw result.error
      return result
    },
    ...
    setDashListRootOrder: (scopeId, order) =>
      metadataCommand(scopeId, (previous) => {
        if (!validOrderArray(order)) throw new TypeError('Invalid Nexus metadata record.')
    ...
    setDashListGroupOrder: (scopeId, groupId, order) =>
      metadataCommand(scopeId, (previous) => {
        if (!validIdentity(groupId)) throw new TypeError('Invalid Nexus metadata record.')
        if (!validOrderArray(order)) throw new TypeError('Invalid Nexus metadata record.')
        const list = previous?.dashList
    ...
    setDashListCollapseOverride: (scopeId, nodeId, collapsed) =>
      metadataCommand(scopeId, (previous) => {
        if (!validIdentity(nodeId) || typeof collapsed !== 'boolean')
          throw new TypeError('Invalid Nexus metadata record.')
        const list = previous?.dashList
    ...
    removeDashListCollapseOverride: (scopeId, nodeId) =>
      metadataCommand(scopeId, (previous) => {
        if (!validIdentity(nodeId)) throw new TypeError('Invalid Nexus metadata record.')
        const list = previous?.dashList
    ...
    updateDashListCollapseOverrides: (scopeId, updates) =>
      metadataCommand(scopeId, (previous) => {
        if (!Array.isArray(updates)) throw new TypeError('Invalid Nexus metadata record.')
    ...
    createPrunePlan: ((options: RootDashListPruneOptions) =>
      createRootPrunePlanInternal(options)) as RootMetadataCommands<NexusResult>['createPrunePlan'],
  ...
  nexus = makeLifecycleFacade(storeImplementation, runtimeController)
  runtimeController.finalizeRoot(nexus as object)
  registerRuntimeController(nexus as object, runtimeController)
  ...
  function applyPersistenceResolution(
    ...
    dispatchNexusSubscribers(affectedChannels)
    return Object.freeze({
      changedFields: Object.freeze(changedFields),
      changedScopeIds: Object.freeze(changedScopeIds),
    })
  ...
  function executePersistenceConflictResolutionPublic(
    ...
    if (record.root !== (nexus as object)) invalidPersistencePlan('foreign-root')
  ...
  function executePersistenceErasePublic(
    ...
    return withWriteLock(() => {
      ...
      if (record.root !== (nexus as object))
        throw new PicodashContractError('invalid-persistence-plan', {
          kind: 'erase',
          reason: 'foreign-root',
        })
    ...
    try {
      ...
      if (!changedFields.length && !changedScopeIds.length) {
        if (options?.beforeDispatch) {
          ...
          refreshScopedChannels(affectedChannels)
          dispatchNexusSubscribers(affectedChannels, options.includeRoot ?? false)
        }
        return resultWithPersistence(successfulResult())
      }
      ...
      dispatchNexusSubscribers(affectedChannels, options?.includeRoot)
      return result
    } finally {
  ...
  function setInteraction(
    ...
    if (!suppressInteractionDispatch) {
      ...
      dispatchNexusSubscribers(affected, false)
    }
    return stored
  }
  ...
  function executeStaleInputOverwriteInternalLocked(plan: object): CoreTransactionResult {
    ...
    if (registry.root !== (nexus as object))
      throw new PicodashContractError('invalid-binding-plan', {
        kind: registry.kind,
        reason: 'foreign-root',
      })
  ...
  function destroyScopeInternalLocked(
    ...
    dispatchNexusSubscribers(affectedChannels, changedScopeIds.length > 0)
    return result
  }
  ...
  function makeScopedNexusSnapshot(scopeId: string): ScopedNexusSnapshot<ValuesOf<Fields>> {
    return freeze({
  ...
  function refreshScopedChannels(affected: Set<ScopedChannel>) {
    for (const channel of affected) channel.snapshot = makeScopedNexusSnapshot(channel.scopeId)
  }

  function dispatchNexusSubscribers(affected: Set<ScopedChannel>, includeRoot = true) {
    diagnosticsRuntime.dispatch([
  ...
  function getScoped(scopeId: string): ScopedNexus<Fields, NexusResult, boolean> {
    validateScopeId(scopeId)
    const cached = scopedRefs.get(scopeId)?.deref() as
      | ScopedNexus<Fields, NexusResult, boolean>
      | undefined
    if (cached) return cached
    const channel =
      ...
      ({
        scopeId,
        snapshot: makeScopedNexusSnapshot(scopeId),
        listeners: new Set<() => void>(),
      } satisfies ScopedChannel)
    const scoped: ScopedNexus<Fields, NexusResult, boolean, boolean> = {
      kind: 'scoped',
      root: nexus,
      scopeId,
      fields,
      ...
      setDashPanelLayout: (layout) => nexus.setDashPanelLayout(scopeId, layout),
      resetDashPanelLayout: () => nexus.resetDashPanelLayout(scopeId),
      setDashListRootOrder: (order) => nexus.setDashListRootOrder(scopeId, order),
      removeDashListRootOrder: () => nexus.removeDashListRootOrder(scopeId),
      setDashListGroupOrder: (groupId, order) =>
        nexus.setDashListGroupOrder(scopeId, groupId, order),
      removeDashListGroupOrder: (groupId) => nexus.removeDashListGroupOrder(scopeId, groupId),
      setDashListCollapseOverride: (nodeId, collapsed) =>
        nexus.setDashListCollapseOverride(scopeId, nodeId, collapsed),
      removeDashListCollapseOverride: (nodeId) =>
        nexus.removeDashListCollapseOverride(scopeId, nodeId),
      updateDashListCollapseOverrides: (updates) =>
        nexus.updateDashListCollapseOverrides(scopeId, updates),
      resetDashListMetadata: () => nexus.resetDashListMetadata(scopeId),
      createPrunePlan: ((options: DashListPruneSelection) =>
        createScopedPrunePlanInternal(
          scopeId,
          options,
        )) as ScopedMetadataCommands<NexusResult>['createPrunePlan'],
  ...
  function classifyPruneSelection(
    ...
    const active = runtimeControllerFor(nexus as object)?.activeDashListNodeIds(scopeId) ?? []
      ...
      try {
        ...
        return rejectedResult([
          Object.freeze({
            ...
            path: freezePath(['scopes', scopeId]),
            message: 'Invalid Nexus metadata.',
          }),
        ])
      }
