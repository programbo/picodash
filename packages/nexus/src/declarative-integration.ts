...
>(
  rootNexus: RootNexus<Fields, Result>,
  providerId?: string,
): DeclarativeIntegrationHost<Fields, Result> {
  return createDeclarativeHost(rootNexus, providerId, false)
}

export function createDeclarativeStandaloneIntegrationHost<
  Fields extends Record<string, FieldLike>,
  Result extends CoreTransactionResult,
>(rootNexus: RootNexus<Fields, Result>): DeclarativeStandaloneIntegrationHost<Fields, Result> {
  return createDeclarativeHost(rootNexus, undefined, true) as DeclarativeStandaloneIntegrationHost<
    Fields,
    Result
  >
}
...
>(
  ...
  const activate = (declaration: DeclarativeEntityMount): ActiveEntity => {
    ...
    if (standalone && declaration.token === standaloneRoot) {
      if (declaration.nexus.root !== rootNexus)
        throw new PicodashContractError('invalid-integration-handle', {
          role: 'host',
          reason: 'foreign-root',
        })
      lease = acquireEntityLease(
        declaration.nexus as ScopedNexus<Record<string, FieldLike>, CoreTransactionResult>,
        { kind: 'dashList' },
      )
    } else if (host) {
    ...
    try {
      if (parent && parent.declaration.nexus.scopeId !== declaration.nexus.scopeId)
