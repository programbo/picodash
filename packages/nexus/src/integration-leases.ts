...
export type NexusEntityKind = 'dashPanel' | 'dashList'
...
export type NexusBindingMode = 'input' | 'display'
...
>(
  scopedNexus: ScopedNexus<Fields, Result>,
  candidate: unknown,
  ...
  try {
    ...
    const fields = scopedNexus.fields as Record<string, unknown>
...
>(
  ...
  const scopedRecord = runtimeScopedViewFor(scopedNexus as object)
  ...
  return controller.withLeaseMutation(() => {
    ...
    const field = ownedField<Fields, Key, Result>(scopedNexus, parsed.field)
...
export function assertBindingHandle<
  ...
  Result extends CoreTransactionResult = CoreTransactionResult,
>(scopedNexus: ScopedNexus<Fields, Result>, handle: unknown): BindingHandle<Fields, Key> {
  const scopedRecord = runtimeScopedViewFor(scopedNexus as object)
  const controller = scopedRecord?.controller
  const owner =
...
export function acquireProviderLease<
  Fields extends Record<string, FieldLike>,
  Result extends CoreTransactionResult = CoreTransactionResult,
>(rootNexus: RootNexus<Fields, Result>, options?: { readonly providerId?: string }): ProviderLease {
  const controller = controllerForRoot(rootNexus as object)
  return controller.withLeaseMutation(() => {
...
export function acquireEntityLease<
  Fields extends Record<string, FieldLike>,
  Result extends CoreTransactionResult = CoreTransactionResult,
>(scopedNexus: ScopedNexus<Fields, Result>, options: EntityLeaseOptions): EntityLease {
  const scopedRecord = runtimeScopedViewFor(scopedNexus as object)
  ...
  return controller.withLeaseMutation(() => {
    ...
    if (conflicting)
      throw new PicodashContractError('scope-host-conflict', { scopeId: scopedNexus.scopeId })
