...
export type NexusOwnedPersistenceConfig<
...
export type PersistenceValueOwner = 'nexus' | 'external'
...
const persistenceContent = (
  ...
  quarantinedScopes: ReadonlyMap<string, PicodashQuarantinedScopeMetadata> = new Map(),
  valueOwner: PersistenceValueOwner = 'nexus',
) => {
...
const encodePersistenceEnvelopeInternal = (
  ...
  const valueOwner = input.valueOwner ?? 'nexus'
  const envelope = {
    kind: 'picodash-nexus-envelope' as const,
...
export function decodePersistenceEnvelope(
  ...
  try {
    ...
    const expectedOwner = expected.valueOwner ?? 'nexus'
    if (owner !== 'nexus' && owner !== 'external') return { ok: false, reason: 'authority' }
    ...
    if (value.kind !== 'picodash-nexus-envelope' || value.formatVersion !== 1)
      return { ok: false, reason: 'format' }
    if (value.nexusId !== expected.nexusId) return { ok: false, reason: 'identity' }
    ...
    if (
      ...
      return { ok: false, reason: 'format' }
    if (owner === 'nexus' && !isRecord(value.values)) return { ok: false, reason: 'values' }
...
/** Decodes a Nexus-owned envelope for driver-free initial hydration. */
...
export function createPersistenceController(
  ...
  const valueOwner = options.valueOwner ?? 'nexus'
