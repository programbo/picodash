...
export class PicodashDocumentOptionsError extends TypeError {
  ...
  constructor(operation: PicodashDocumentOptionOperation, reason: PicodashDocumentOptionReason) {
    super('Invalid Nexus document options.')
...
function canonicalDocument(value: unknown): PicodashDocument {
  ...
  const expected =
    kind === 'root'
      ? ['formatVersion', 'kind', 'nexusId', 'schemaVersion', 'fields', 'scopes']
      : kind === 'scope'
        ? ['formatVersion', 'kind', 'nexusId', 'schemaVersion', 'scopeId', 'fields', 'scopes']
        : []
  ...
  if (!validIdentity(captured.nexusId)) throw new PicodashDocumentError('identity')
  ...
  const base = {
    ...
    nexusId: captured.nexusId as string,
...
/** Alias matching the concise configuration terminology used by Nexus setup. */
export const normalizeExportPolicy = normalizePicodashExportPolicy

/** A real root-owned Nexus field handle; the private brand is retained at the public type boundary. */
...
export type PicodashRootImportOptions<Values extends object = Record<string, PicodashJsonValue>> =
  Readonly<{
    readonly allowForeignNexus?: boolean
...
export function normalizePicodashImportOptions(
  ...
  for (const key of ['allowForeignNexus', 'createMissingScopes'] as const)
  ...
  if (Object.hasOwn(record, 'targetScopeId') && !validIdentity(own(record, 'targetScopeId')))
    optionError('import-analysis', 'invalid-target')
  return freeze({
    allowForeignNexus: (own(record, 'allowForeignNexus') as boolean | undefined) ?? false,
...
export function normalizePicodashImportPlanReview(
  ...
  if (value.targetScopeId !== undefined && !validIdentity(value.targetScopeId))
    throw new TypeError('Invalid import plan review.')
  if (typeof value.foreignNexus !== 'boolean') throw new TypeError('Invalid import plan review.')
  ...
  return freeze({
    ...
    foreignNexus: value.foreignNexus,
  })
}
...
/** Run the complete schema chain for a document without invoking Nexus callbacks. */
