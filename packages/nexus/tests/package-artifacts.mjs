...
export async function checkNexusPackage(root = packageRoot) {
  const errors = []
  const manifest = await readJson(path.join(root, 'package.json'))
  if (manifest.name !== '@picodash/nexus') {
    errors.push(`package name must be @picodash/nexus, received ${JSON.stringify(manifest.name)}`)
  }
  if (
    manifest.description !==
    'Typed state hub for settings panels, inspectors, and control dashboards.'
  ) {
    errors.push('description does not match Nexus package positioning')
  }
  if (
    JSON.stringify(
      Object.fromEntries(
...
if (import.meta.main) {
  const errors = await checkNexusPackage()
  assert.deepEqual(
    errors,
    [],
    `Nexus package artifact validation failed:\n- ${errors.join('\n- ')}`,
  )
  ...
  assert.doesNotMatch(rootTypes, /\b(?:Root|Scoped)Store\b/)
  assert.doesNotMatch(rootTypes, /createPicodashStore/)
  ...
  const scopedMarker = rootTypes.includes('interface ScopedNexusBase')
  ...
  assert.equal('createPicodashStore' in rootModule, false)
  ...
  const artifactNexus = rootModule.createPicodashNexus({
    valueOwner: 'nexus',
    fields: { value: { defaultValue: 1 } },
  })
  const artifactScoped = artifactNexus.scope('artifact')
  assert.equal(typeof artifactNexus.diagnostics.getState, 'function')
  assert.equal(typeof artifactScoped.diagnostics.subscribe, 'function')
  artifactNexus.destroy()
  assert.throws(() => artifactNexus.getState(), /use-after-destroy/)
  assert.throws(() => artifactScoped.getState(), /use-after-destroy/)

  const builtBoundaryNexus = rootModule.createPicodashNexus({
    valueOwner: 'nexus',
    fields: { value: { defaultValue: 1 } },
  })
  function BuiltContextProbe() {
    return createElement(
      'output',
      null,
      reactModule.usePicodashRootNexus() === builtBoundaryNexus ? 'same-root' : 'different-root',
    )
  }
  const builtContextMarkup = renderToStaticMarkup(
    createElement(integrationModule.PicodashNexusProviderBoundary, {
      nexus: builtBoundaryNexus,
      children: createElement(BuiltContextProbe),
    }),
  )
  assert.equal(builtContextMarkup, '<output>same-root</output>')
  builtBoundaryNexus.destroy()

  const builtStandaloneNexus = rootModule.createPicodashNexus({
    valueOwner: 'nexus',
    fields: { value: { defaultValue: 1 } },
  })
  ...
  const builtStandaloneMarkup = renderToStaticMarkup(
    createElement(integrationModule.PicodashNexusEntityBoundary, {
      nexus: builtStandaloneNexus.scope('artifact-standalone'),
      kind: 'dashList',
  ...
  const externalNexus = rootModule.createPicodashNexus({
    valueOwner: 'external',
    fields: { value: { defaultValue: 1 } },
    adapter: externalAdapter,
  })
  assert.equal(externalNexus.setValues({ value: 2 }).ok, true)
  assert.equal(externalNexus.getState().values.value, 2)
  externalNexus.destroy()
  ...
  const persistentNexus = rootModule.createPicodashNexus({
    valueOwner: 'nexus',
    nexusId: 'artifact-persistence',
    schemaVersion: 1,
    fields: { value: { defaultValue: 1 } },
    persistence: {
      storageKey: 'state',
  ...
  assert.equal(persistentNexus.setValues({ value: 2 }).persistence, 'saved')
  assert.equal(persistentNexus.persistence.getState().status, 'clean')
  persistentNexus.destroy()
  assert.equal(removeCalls, 0)

  let externalPersistentValue = 1
  const externalPersistentListeners = new Set()
  const externalPersistentNexus = rootModule.createPicodashNexus({
    valueOwner: 'external',
    nexusId: 'artifact-external-persistence',
    schemaVersion: 1,
    fields: { value: { defaultValue: 1 } },
    adapter: {
  ...
  assert.equal(externalPersistentNexus.setValues({ value: 2 }).persistence, 'unchanged')
  assert.equal(
    externalPersistentNexus.setDashListRootOrder('artifact', ['item']).persistence,
    'saved',
  )
  ...
  externalPersistentNexus.destroy()

  // Exercise the negative boundary with a temporary reachable React import.
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), 'picodash-nexus-artifact-'))
  try {
    ...
    await writeFile(
      ...
      JSON.stringify({
        name: '@picodash/nexus',
        description: 'Typed state hub for settings panels, inspectors, and control dashboards.',
    ...
    const negativeErrors = await checkNexusPackage(temporaryRoot)
  ...
  console.log(
    'Nexus package artifacts are valid; reachable React-facing import rejection is covered.',
  )
}
