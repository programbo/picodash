...
import {
  acquireNexusScopeRelationship,
  createNexusScopeModel,
  destroyNexusScopeState,
  releaseNexusScopeRelationship,
  setNexusScopeMetadata,
  clearNexusScopeMetadata,
} from './support/nexus-scope-model.ts'
...
describe('Nexus declarative integration leases', () => {
  it('acquires frozen opaque provider/entity handles and enforces provider IDs', () => {
    ...
    expect(failure(() => acquireProviderLease(nexus)).context).toEqual({ providerId: 'default' })
    expect(failure(() => acquireProviderLease(nexus, { providerId: ' bad ' })).context).toEqual({
      reason: 'surrounding-whitespace',
    })
    const panel = acquireEntityLease(nexus.scope('panel'), { kind: 'dashPanel', host: provider })
  ...
  it('validates hostile entity records before examining host handles', () => {
    const nexus = makeNexus()
    const provider = acquireProviderLease(nexus)
    expect(failure(() => acquireEntityLease(nexus.scope('x'), null as never)).context).toEqual({
      reason: 'not-object',
    })
    expect(
      failure(() =>
        acquireEntityLease(nexus.scope('x'), { kind: 'dashPanel', extra: true } as never),
      ).context,
    ...
    expect(failure(() => acquireEntityLease(nexus.scope('x'), hostile as never)).context).toEqual({
      reason: 'accessor-property',
    })
    expect(invoked).toBe(false)
    expect(
      failure(() => acquireEntityLease(nexus.scope('x'), { kind: 'dashPanel' } as never)).context,
    ).toEqual({ reason: 'host-required' })
    provider.release()
  })

  it('rejects forged scoped-nexus lookalikes without trusting root or scope fields', () => {
    const nexus = makeNexus()
    const provider = acquireProviderLease(nexus)
    const forged = { root: nexus, scopeId: 'PRIVATE_SENTINEL' } as never
  ...
  it('normalizes hostile option proxies without invoking accessors or leaking trap data', () => {
    ...
    const error = failure(() => acquireEntityLease(nexus.scope('proxy'), throwingProxy as never))
  ...
  it('holds the write lock while lease options are reflected and registrations are committed', () => {
    const nexus = makeNexus()
    const scoped = nexus.scope('locked')
    const nestedErrors: string[] = []
    const guardedOptions = <Options extends object>(options: Options): Options =>
      new Proxy(options, {
        ownKeys(target) {
          nestedErrors.push(failure(() => nexus.setValue(nexus.fields.value, 2)).code)
          return Reflect.ownKeys(target)
        },
      })

    const provider = acquireProviderLease(nexus, guardedOptions({ providerId: 'locked-provider' }))
  ...
  it('supports Strict Mode-style release and reacquisition generations', () => {
    const nexus = makeNexus()
    const first = acquireProviderLease(nexus)
    first.release()
    const second = acquireProviderLease(nexus)
    const entity = acquireEntityLease(nexus.scope('strict'), { kind: 'dashList', host: second })
    entity.release()
    entity.release()
    second.release()
    second.release()
  ...
  it('covers provider lexical validation and integration handle taxonomy', () => {
    ...
    for (const [providerId, reason] of [
      ...
      const error = failure(() => acquireProviderLease(nexus, { providerId: providerId as never }))
      expect(error.code).toBe('invalid-provider-id')
      expect(error.context).toEqual({ reason })
    }
    const provider = acquireProviderLease(nexus)
    const standalone = acquireEntityLease(nexus.scope('standalone'), { kind: 'dashList' })
    expect(
      failure(() =>
        acquireEntityLease(nexus.scope('standalone'), { kind: 'dashPanel', host: provider }),
      ).context,
    ).toEqual({ scopeId: 'standalone' })
    standalone.release()
    const panel = acquireEntityLease(nexus.scope('panel'), { kind: 'dashPanel', host: provider })
    const list = acquireEntityLease(nexus.scope('list'), { kind: 'dashList', host: panel })
    expect(failure(() => provider.release()).context).toEqual({ leaseKind: 'provider' })
    expect(
      failure(() =>
        acquireEntityLease(nexus.scope('other'), { kind: 'dashList', host: {} as never }),
      ).context,
    ).toEqual({
      role: 'host',
      reason: 'wrong-kind',
  ...
  it('matches the scope-model oracle for generated metadata and directed destruction', () => {
    const nexus = makeNexus()
    const provider = acquireProviderLease(nexus)
    const parent = acquireEntityLease(nexus.scope('a'), { kind: 'dashPanel', host: provider })
    const middle = acquireEntityLease(nexus.scope('b'), { kind: 'dashList', host: parent })
    const leaf = acquireEntityLease(nexus.scope('c'), { kind: 'dashList', host: parent })
    const firstEdge = acquireRelationshipLease(parent, middle)
    const secondEdge = acquireRelationshipLease(middle, leaf)
    let model = createNexusScopeModel({ value: 1 })
    ...
    modelState = acquireNexusScopeRelationship(modelState, 'a-b', 'a', 'b').state
    modelState = acquireNexusScopeRelationship(modelState, 'b-c', 'b', 'c').state
    const destroyedModel = destroyNexusScopeState(modelState, 'a', true)
    const expected = [...modelState.durableScopeIds]
      .filter((scopeId) => !destroyedModel.durableScopeIds.has(scopeId))
      .sort()
    const actual = nexus.destroyScope('a', { includeDescendants: true })
  ...
  it('enforces uniqueness, host affinity, graph parent conflicts, cycles, and teardown', () => {
    ...
    expect(
      failure(() =>
        acquireEntityLease(nexus.scope('parent'), { kind: 'dashPanel', host: provider }),
      ).code,
    ).toBe('duplicate-entity')
    const otherProvider = acquireProviderLease(nexus, { providerId: 'other' })
    expect(
      failure(() =>
        acquireEntityLease(nexus.scope('parent'), { kind: 'dashList', host: otherProvider }),
      ).code,
  ...
  it('treats entity-hosted children as dependents even without a relationship edge', () => {
    const nexus = makeNexus()
    const provider = acquireProviderLease(nexus)
    const host = acquireEntityLease(nexus.scope('host'), { kind: 'dashPanel', host: provider })
    const hosted = acquireEntityLease(nexus.scope('hosted'), { kind: 'dashList', host })
  ...
  it('keys graph parentage by scope while retaining generation leases', () => {
    const nexus = makeNexus()
    const provider = acquireProviderLease(nexus)
    const parentPanel = acquireEntityLease(nexus.scope('parent'), {
      kind: 'dashPanel',
      host: provider,
    })
    const parentList = acquireEntityLease(nexus.scope('parent'), {
      kind: 'dashList',
      host: provider,
    })
    const childPanel = acquireEntityLease(nexus.scope('child'), {
      kind: 'dashPanel',
      host: provider,
    })
    const childList = acquireEntityLease(nexus.scope('child'), {
      kind: 'dashList',
      host: provider,
    })
    ...
    const otherParent = acquireEntityLease(nexus.scope('other'), {
      kind: 'dashPanel',
      host: provider,
    })
    ...
    const grandchild = acquireEntityLease(nexus.scope('grandchild'), {
      kind: 'dashList',
      host: provider,
    })
  ...
  it('keeps active descendants available to destroyScope without deleting leases or values', () => {
    ...
    const edge = acquireRelationshipLease(parent, child)
    const secondEdge = acquireRelationshipLease(parent, child)
    nexus.setDashPanelLayout('parent', {
      placement: { mode: 'floating', disposition: { kind: 'free' } },
      preferredPosition: { x: 1, y: 2 },
    })
    nexus.setDashListRootOrder('child', ['item'])
    const valueRef = nexus.getState().values
    expect(nexus.destroyScope('parent', { includeDescendants: true })).toEqual({
      ok: true,
      changedFields: [],
      changedScopeIds: ['child', 'parent'],
    })
    expect(nexus.getState().values).toBe(valueRef)
  ...
  property.prop([
    ...
    let modelState = createNexusScopeModel({ value: 1 }).state
    ...
    for (let index = 0; index < commands.length; index += 1) {
      ...
      if (command.kind === 'set') {
        ...
        const result = nexus.setDashListRootOrder(command.scope, [command.scope])
        expect(result.ok ? result.changedScopeIds : []).toEqual(had ? [] : [command.scope])
        modelState = setNexusScopeMetadata(modelState, command.scope)
      } else if (command.kind === 'remove') {
        nexus.removeDashListRootOrder(command.scope)
        modelState = clearNexusScopeMetadata(modelState, command.scope)
      } else if (command.kind === 'destroy') {
        const before = new Set(modelState.durableScopeIds)
        const next = destroyNexusScopeState(modelState, command.scope, command.descendants)
        const result = nexus.destroyScope(command.scope, {
        ...
        const transition = acquireNexusScopeRelationship(
          modelState,
          leaseId,
          parentScopeId,
          childScopeId,
        ...
        if (releasedId) modelState = releaseNexusScopeRelationship(modelState, releasedId).state
      }
      expect([...nexus.getState().scopes.keys()].sort()).toEqual(
