...
describe('DashList node leases and prune plans', () => {
  it('keeps node presence private, generation-safe, and root-destroying', () => {
    ...
    nexus.subscribe(() => rootNotifications++)
  ...
  it('validates node options without invoking accessors', () => {
    const scoped = makeNexus().scope('scope')
  ...
  it('holds the write lock while prune options and candidates are captured', () => {
    ...
    const options = new Proxy(
      { mode: 'review' as const },
      {
        ownKeys(target) {
          try {
            scoped.setValue(nexus.fields.value, 9)
          } catch (error) {
            nestedError = error
          }
  ...
  it('fences plans by target metadata and active membership, then prunes only metadata', () => {
    ...
    scoped.setValue(nexus.fields.value, 2)
  ...
  it('rejects forged, review, and foreign plans without exposing runtime state', () => {
    ...
    const foreignNexus = makeNexus()
    const foreignScope = foreignNexus.scope('scope')
  ...
  it('executes an empty prune plan as a no-op for quarantined metadata', () => {
    ...
    expect(nexus.metadataRecovery.getState().quarantinedScopes.has('scope')).toBe(true)
    const scoped = nexus.scope('scope')
