...
describe('test-only fixture foundation', () => {
  ...
  it('models root values and scope metadata with immutable semantic no-ops', () => {
    ...
    const withMetadata = setNexusScopeMetadata(model.state, 'a')
    expect(withMetadata.values).not.toBe(values)
    expect(withMetadata.durableScopeIds.has('a')).toBe(true)
    const cleared = clearNexusScopeMetadata(withMetadata, 'a')
  ...
  it('enforces one parent, acyclicity, named release, and descendant state destruction', () => {
    const resolved = createNexusScopeModel({ count: 1 }).resolveScope('root').state
    const leaseWithoutMetadata = acquireNexusScopeRelationship(
      resolved,
      'ephemeral',
      'root',
      'ephemeral-child',
    ...
    let state = setNexusScopeMetadata(createNexusScopeModel({ count: 1 }).state, 'root')
    state = setNexusScopeMetadata(state, 'child')
    state = setNexusScopeMetadata(state, 'grandchild')
    const rootLease = acquireNexusScopeRelationship(state, 'root-child', 'root', 'child')
    expect(rootLease.reason).toBeUndefined()
    state = rootLease.state
    const sameEdgeLease = acquireNexusScopeRelationship(state, 'root-child-2', 'root', 'child')
    expect(sameEdgeLease.reason).toBeUndefined()
    state = sameEdgeLease.state
    const childLease = acquireNexusScopeRelationship(
      state,
      'child-grandchild',
      'child',
      'grandchild',
    ...
    expect(acquireNexusScopeRelationship(state, 'other-parent', 'other', 'child').reason).toBe(
      'parent-already-set',
    )
    expect(acquireNexusScopeRelationship(state, 'cycle', 'grandchild', 'root').reason).toBe('cycle')
    ...
    expect(releaseNexusScopeRelationship(state, 'missing').state).toBe(state)
    const releasedOne = releaseNexusScopeRelationship(state, 'root-child')
    expect(releasedOne.reason).toBeUndefined()
    expect(destroyNexusScopeState(releasedOne.state, 'root', true).durableScopeIds.size).toBe(0)
    const releasedFinal = releaseNexusScopeRelationship(releasedOne.state, 'root-child-2')
    expect(releasedFinal.reason).toBeUndefined()
    const noDescendants = destroyNexusScopeState(releasedFinal.state, 'root', true)
    expect(noDescendants.durableScopeIds.has('root')).toBe(false)
    expect(noDescendants.durableScopeIds.has('child')).toBe(true)
    const destroyed = destroyNexusScopeState(state, 'root', true)
    ...
    expect(destroyNexusScopeState(state, 'root', false).durableScopeIds.has('child')).toBe(true)
    expect(acquireNexusScopeRelationship(state, 'other-parent', 'other', 'child').state).toBe(state)
  })
