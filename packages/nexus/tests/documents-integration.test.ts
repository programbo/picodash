...
const createNexus = (nexusId: string) =>
  createPicodashNexus({
    valueOwner: 'nexus',
    nexusId,
    schemaVersion: 1,
...
describe('Nexus document namespace integration', () => {
  it('snapshots Nexus document identity for the root lifetime', () => {
    const config = {
      valueOwner: 'nexus' as const,
      nexusId: 'documents-identity-alpha',
      schemaVersion: 1,
      fields: { value: { defaultValue: 1 } },
      export: { documents: { defaultFieldPolicy: 'include' as const } },
    }
    const nexus = createPicodashNexus(config)
    config.nexusId = 'documents-identity-beta'
    config.schemaVersion = 2

    const exported = nexus.documents.executeExport(nexus.documents.createExportPlan())
    expect(exported.ok).toBe(true)
    if (!exported.ok) return
    expect(exported.document.nexusId).toBe('documents-identity-alpha')
  ...
  it('snapshots migration callbacks for the root lifetime', () => {
    const source = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-migration-source',
      schemaVersion: 1,
      fields: { value: { defaultValue: 1 } },
    ...
    const target = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-migration-target',
      schemaVersion: 2,
    ...
    const analysis = target.documents.analyzeImport(exported.document, { allowForeignNexus: true })
  ...
  it('holds the write lock while import analysis validators run', () => {
    const source = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-analysis-source',
      schemaVersion: 1,
    ...
    const target = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-analysis-target',
      schemaVersion: 1,
      fields: {
        value: {
          defaultValue: 1,
    ...
    expect(() =>
      target.documents.analyzeImport(exported.document, { allowForeignNexus: true }),
  ...
  it('holds the write lock while import execution validators run', () => {
    const source = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-reentrant-source',
      schemaVersion: 1,
      fields: { value: { defaultValue: 2 }, other: { defaultValue: 0 } },
      export: { documents: { defaultFieldPolicy: 'include' } },
    })
    ...
    const target = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-reentrant-target',
      schemaVersion: 1,
      fields: {
        value: {
          defaultValue: 1,
  ...
  it('fences entity release while scoped import validators run', () => {
    const source = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-entity-source',
      schemaVersion: 1,
    ...
    const target = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-entity-target',
      schemaVersion: 1,
      fields: {
        value: {
          defaultValue: 1,
  ...
  it('fences lease acquisition while import validators run', () => {
    const source = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-acquire-source',
      schemaVersion: 1,
      fields: { value: { defaultValue: 2 } },
      export: { documents: { defaultFieldPolicy: 'include' } },
    })
    ...
    const target = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-acquire-target',
      schemaVersion: 1,
      fields: {
        value: {
          defaultValue: 1,
  ...
  it('exports root and scoped projections with policy and one-use plans', () => {
    const nexus = createNexus('documents-integration')
    const rootPlan = nexus.documents.createExportPlan()
    const rootResult = nexus.documents.executeExport(rootPlan)
    ...
    const scopedPlan = nexus.scope('settings').documents.createExportPlan({
      includeDescendants: false,
      fields: [nexus.fields.value],
    })
    const scopedResult = nexus.scope('settings').documents.executeExport(scopedPlan)
  ...
  it('guards cached root and scoped document capabilities after destruction', () => {
    const nexus = createNexus('documents-destroyed-capability')
    const rootDocuments = nexus.documents
    const scopedDocuments = nexus.scope('scope').documents
    nexus.destroy()
    for (const action of [
  ...
  it('requires explicit promotion confirmation and does not consume on malformed execution options', () => {
    const nexus = createNexus('documents-promotion')
    const plan = nexus.documents.createExportPlan({
      includeDescendants: false,
      fields: [nexus.fields.secret],
      promoteFields: [nexus.fields.secret],
    })
    expect(() => nexus.documents.executeExport(plan, undefined)).toThrowError(
    ...
    const confirmation = new Proxy(
      { confirmRedactedPromotion: true as const },
      {
        ownKeys(target) {
          try {
            nexus.setValue(nexus.fields.value, 99)
          } catch (error) {
            nestedError = error
          }
    ...
    const result = nexus.documents.executeExport(plan, confirmation)
  ...
  it('imports atomically, fences stale plans, and rejects foreign identity without permission', () => {
    const source = createNexus('documents-source')
    ...
    const target = createNexus('documents-target')
    ...
    const fresh = target.documents.analyzeImport(sourceResult.document, { allowForeignNexus: true })
  ...
  it('rejects root-document retargeting and stales plans when an active-only target disappears', () => {
    const nexus = createNexus('documents-scope-existence')
    const exported = nexus.documents.executeExport(nexus.documents.createExportPlan())
    expect(exported.ok).toBe(true)
    if (!exported.ok) return
    expect(() =>
      nexus.documents.analyzeImport(exported.document, { targetScopeId: 'transient' }),
    ).toThrowError(
    ...
    const transient = nexus.scope('transient')
    const lease = acquireEntityLease(transient, { kind: 'dashList' })
    const analysis = nexus.documents.analyzeImport(scopeDocument, {
      targetScopeId: 'transient',
    })
    expect(analysis.ok).toBe(true)
    if (!analysis.ok) return
    lease.release()
    expect(nexus.documents.executeImport(analysis.plan)).toMatchObject({
      ok: false,
  ...
  it('validates and canonicalizes imported values during analysis', () => {
    const source = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-analysis-pipeline',
      schemaVersion: 1,
    ...
    const rejectingTarget = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-analysis-pipeline',
      schemaVersion: 1,
      fields: {
  ...
  it('commits a non-idempotent schema canonicalizer exactly once', () => {
    const source = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-single-canonicalization',
      schemaVersion: 1,
  ...
  it('stales import plans when tracked target metadata changes', () => {
    const source = createNexus('documents-metadata-freshness')
    const target = createNexus('documents-metadata-freshness')
  ...
  it('fences only mapped import field revisions', () => {
    const source = createNexus('documents-value-revisions')
    const target = createNexus('documents-value-revisions')
  ...
  it('fences scoped plans to their receiver target', () => {
    const nexus = createNexus('documents-target-fence')
    const plan = nexus.scope('one').documents.createExportPlan({ includeDescendants: false })
    try {
      nexus.scope('two').documents.executeExport(plan)
  ...
  it('imports through Nexus-owned persistence and preserves the saved result', () => {
    const source = createNexus('documents-persistence-import')
    const exported = source.documents.executeExport(source.documents.createExportPlan())
    expect(exported.ok).toBe(true)
    if (!exported.ok) return
    ...
    const target = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-persistence-import',
      schemaVersion: 1,
  ...
  it('imports as one atomic external-adapter batch and leaves state unchanged on failure', () => {
    const source = createNexus('documents-adapter-import')
    const adapter = createExternalAdapter({ value: 10, secret: 'target' })
    const target = createPicodashNexus({
      valueOwner: 'external',
      nexusId: 'documents-adapter-import',
      schemaVersion: 1,
  ...
  it('runs document schema migration before import overlay', () => {
    const source = createNexus('documents-migration-import')
    const exported = source.documents.executeExport(source.documents.createExportPlan())
    expect(exported.ok).toBe(true)
    if (!exported.ok) return
    const target = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-migration-import',
      schemaVersion: 2,
      fields: { value: { defaultValue: 0 }, secret: { defaultValue: 'target' } },
      migrations: {
        1: (payload) => ({
  ...
  it('tracks mapped quarantine freshness, clears imported quarantine, and ignores unrelated quarantine changes', () => {
    const source = createNexus('documents-quarantine-import')
    const panel = {
      placement: { mode: 'floating', disposition: { kind: 'free' } },
    ...
    const quarantineEnvelope = JSON.stringify({
      ...
      formatVersion: 1,
      nexusId: 'documents-quarantine-import',
      schemaVersion: 1,
      revision: 1,
    ...
    const staleTarget = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-quarantine-import',
      schemaVersion: 1,
      fields: { value: { defaultValue: 0 }, secret: { defaultValue: 'target' } },
    ...
    const addedQuarantineTarget = createPicodashNexus({
      valueOwner: 'nexus',
      nexusId: 'documents-quarantine-import',
      schemaVersion: 1,
      fields: { value: { defaultValue: 0 }, secret: { defaultValue: 'target' } },
