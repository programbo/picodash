...
describe('Nexus beta migration and metadata recovery', () => {
  it('exposes recovery only for identified Nexus roots and shares it with scopes', () => {
  ...
  it('rejects accessor, symbol, and out-of-range migration configuration keys', () => {
    ...
    expect(() =>
      createPicodashNexus({ ...config(createMemoryPersistence()), migrations: accessor }),
    ...
    expect(() =>
      createPicodashNexus({ ...config(createMemoryPersistence()), migrations: symbol }),
    ).toThrowError(expect.objectContaining({ code: 'invalid-configuration' }))
    expect(() =>
      createPicodashNexus({
  ...
  it('runs a frozen detached N to N+1 payload before current projection', () => {
    ...
    expect(nexus.getState().values).toMatchObject({ value: 2, added: 8 })
  ...
  it('diagnoses ignored persisted fields and recovers after a verified projection write', () => {
    ...
    const nexus = createPicodashNexus(config(persistence))
    const diagnostic = [...nexus.diagnostics.getState().current.values()].find(
  ...
  it('quarantines one invalid scope, blocks ordinary writes, and replaces deliberately', () => {
    ...
    const nexus = createPicodashNexus(config(persistence))
    const recovery = nexus.metadataRecovery
    ...
    expect(nexus.getState().scopes.has('good-scope')).toBe(true)
    expect(
      nexus.setDashPanelLayout('bad-scope', {
        placement: { mode: 'floating', disposition: { kind: 'free' } },
        preferredPosition: { x: 1, y: 2 },
      }),
    ).toMatchObject({ ok: false, error: { issues: [{ code: 'quarantined_metadata' }] } })
    expect(nexus.setValues({ value: 2 })).toMatchObject({ ok: true, persistence: 'saved' })
  ...
  it('destroys quarantine-only scope state and publishes recovery', () => {
    ...
    nexus.metadataRecovery.subscribe(listener)
    expect(nexus.destroyScope('quarantine-only')).toMatchObject({
      ok: true,
      changedScopeIds: ['quarantine-only'],
      persistence: 'saved',
    })
    expect(listener).toHaveBeenCalledTimes(1)
    expect(nexus.metadataRecovery.getState().quarantinedScopes.has('quarantine-only')).toBe(false)
    expect(
      [...nexus.diagnostics.getState().current.values()].some(
  ...
  it('recovers metadata recovery subscriber diagnostics after a successful publication', () => {
    ...
    nexus.metadataRecovery.subscribe(() => {
      if (shouldThrow) throw new Error('private')
    })
    expect(nexus.metadataRecovery.replaceScope('first', null)).toMatchObject({ ok: true })
    expect(
      [...nexus.diagnostics.getState().current.values()].some(
        (entry) =>
    ...
    shouldThrow = false
    expect(nexus.metadataRecovery.replaceScope('second', null)).toMatchObject({ ok: true })
    expect(
      [...nexus.diagnostics.getState().current.values()].some(
