...
describe('Nexus root kernel', () => {
  test('creates frozen nominal handles and a stable empty snapshot', () => {
    const nexus = createPicodashNexus({
      valueOwner: 'nexus',
      fields: { count: { defaultValue: 1 } },
    })
    const first = nexus.getState()
    expect(Object.keys(nexus.fields.count)).toEqual(['key'])
    expect(Object.isFrozen(nexus.fields.count)).toBe(true)
    expect(Object.isFrozen(first)).toBe(true)
    expect(Object.isFrozen(first.values)).toBe(true)
    expect(first.scopes.size).toBe(0)
    expect(nexus.setValues({})).toEqual({ ok: true, changedFields: [], changedScopeIds: [] })
    expect(nexus.getState()).toBe(first)
  })
  ...
  test('canonicalizes a whole candidate before every field validator and root validation', () => {
    ...
    seen.length = 0
    const result = nexus.setValues({ second: 20, first: 10 })
    expect(result).toEqual({ ok: true, changedFields: ['first', 'second'], changedScopeIds: [] })
    expect(seen).toEqual([
      { first: 10, second: 20 },
      { first: 10, second: 20 },
    ])
    expect(nexus.getState().values).toEqual({ first: 10, second: 20 })
  })
  ...
  test('rejects atomically, including unknown and inherited keys', () => {
    ...
    inherited.count = 2
    const result = nexus.setValues(inherited as never)
    expect(result.ok).toBe(true)
    expect(nexus.getState().values.count).toBe(2)
    const unknown = nexus.setValues({ nope: 3 } as never)
    expect(unknown.ok).toBe(false)
    expect(nexus.getState()).not.toBe(before)
    expect(nexus.getState().values.title).toBe('a')
    if (!unknown.ok)
  ...
  test('reruns validation for semantic no-ops but does not notify', () => {
    ...
    validations = 0
    const empty = nexus.setValues({})
    expect(empty).toMatchObject({ ok: true, changedFields: [] })
    expect(validations).toBe(0)
    const result = nexus.setValues({ count: 1 })
  ...
  test('notifies all listeners despite exceptions and isolates reentrant writes', () => {
    ...
    nexus.subscribe(() => {
      calls.push('first')
      expect(() => nexus.setValue(nexus.fields.count, 3)).toThrowError(
  ...
  test('preserves the same transaction error for OrThrow', () => {
    ...
    const result = nexus.setValue(nexus.fields.count, 2)
    expect(result.ok).toBe(false)
    if (!result.ok) {
      expect(() => nexus.setValueOrThrow(nexus.fields.count, 2)).toThrow(result.error)
  ...
  test('accepts empty stores and rejects invalid construction identities and values', () => {
    const empty = createPicodashNexus({ valueOwner: 'nexus', fields: {} })
    ...
    for (const nexusId of [
      ...
      expect(() => createPicodashNexus({ valueOwner: 'nexus', nexusId, fields: {} })).toThrowError(
        PicodashContractError,
      )
    for (const key of [
      '',
      ...
      expect(() => createPicodashNexus({ valueOwner: 'nexus', fields })).toThrowError(
        PicodashContractError,
      )
    }

    expect(() =>
      createPicodashNexus({ valueOwner: 'nexus', fields: {}, schemaVersion: 1 }),
    ).toThrowError(PicodashContractError)
    expect(() =>
      createPicodashNexus({ valueOwner: 'nexus', fields: {}, nexusId: 'app', schemaVersion: 0 }),
    ).toThrowError(PicodashContractError)
    expect(() =>
      createPicodashNexus({ valueOwner: 'nexus', fields: {}, nexusId: 'app', schemaVersion: -1 }),
    ).toThrowError(PicodashContractError)
    expect(() =>
      createPicodashNexus({ valueOwner: 'nexus', fields: {}, nexusId: 'app', schemaVersion: 1.5 }),
    ).toThrowError(PicodashContractError)
    expect(() =>
      createPicodashNexus({
        valueOwner: 'nexus',
    ...
    expect(() =>
      createPicodashNexus({ valueOwner: 'nexus', fields: {}, initialValues: [] as never }),
    ).toThrowError(PicodashContractError)
  })
  ...
  test('captures schema, field, and root callback references at construction', () => {
    ...
    expect(nexus.setValues({ value: 2 })).toMatchObject({ ok: true, changedFields: ['value'] })
  ...
  test('runs root validation once with source and absolute paths', () => {
    ...
    const initial = createPicodashNexus({
      valueOwner: 'nexus',
  ...
  test('rejects mixed batches atomically and ignores inherited keys', () => {
    ...
    const before = nexus.getState()
    const unknown = nexus.setValues({ first: 5, toString: 7 } as never)
    expect(unknown.ok).toBe(false)
    expect(nexus.getState()).toBe(before)
    expect(notifications).toBe(0)
    const rejected = nexus.setValues({ first: 5, second: 9 })
    expect(rejected.ok).toBe(false)
    expect(nexus.getState()).toBe(before)
    expect(notifications).toBe(0)
    const invalidJson = nexus.setValues({ first: new Date() } as never)
    ...
    inherited.first = 3
    expect(nexus.setValues(inherited as never)).toMatchObject({
      ok: true,
      changedFields: ['first'],
    })
    expect(nexus.getState().values).toEqual({ first: 3, second: 2 })
  })
  ...
  test('freezes every public result and preserves no-op snapshot references', () => {
    ...
    const initial = nexus.getState()
    let notifications = 0
    const unsubscribe = nexus.subscribe(() => {
      notifications += 1
    })
    expect(Object.isFrozen(nexus)).toBe(true)
    expect(Object.isFrozen(nexus.fields)).toBe(true)
    ...
    expect(nexus.getState()).toBe(initial)
    const noop = nexus.setValues({ value: 1 })
    expect(Object.isFrozen(noop)).toBe(true)
    expect(nexus.getState()).toBe(initial)
    expect(notifications).toBe(0)
    const changed = nexus.setValues({ value: 2 })
    ...
    const invalid = nexus.setValues({ missing: 1 } as never)
    ...
    const invalidRecord = nexus.setValues([] as never)
  ...
  test('validates one changed multi-field batch, notifies once, and supports OrThrow success', () => {
    ...
    expect(notifications).toBe(1)
    const thrown = nexus.setValueOrThrow(nexus.fields.first, 11)
  ...
  test('nested writes from every validator are rejected as reentrant writes', () => {
    let schemaNexus!: ReturnType<typeof createPicodashNexus>
    const schema = {
      '~standard': {
        version: 1 as const,
        vendor: 'reentrant-test',
        validate: (value: unknown) => {
          if (value === 2) schemaNexus.setValues({ value: 3 })
          return { value, issues: undefined }
        },
      },
    } as unknown as StandardSchemaV1<unknown, number>
    schemaNexus = createPicodashNexus({
      valueOwner: 'nexus',
      fields: { value: { defaultValue: 1, schema } },
    })
    expect(() => schemaNexus.setValues({ value: 2 })).toThrowError(
      expect.objectContaining({ code: 'reentrant-write' }),
    )

    let fieldNexus!: ReturnType<typeof createPicodashNexus>
    fieldNexus = createPicodashNexus({
      valueOwner: 'nexus',
      fields: {
        value: {
          defaultValue: 1 as number,
          validate: (value) => {
            if (value === 2) fieldNexus.setValues({ value: 3 })
            return []
          },
        },
      },
    })
    expect(() => fieldNexus.setValues({ value: 2 })).toThrowError(
      expect.objectContaining({ code: 'reentrant-write' }),
    )

    let rootNexus!: ReturnType<typeof createPicodashNexus>
    rootNexus = createPicodashNexus({
      valueOwner: 'nexus',
      fields: { value: { defaultValue: 1 as number } },
      validateValues: (values) => {
        if (values.value === 2) rootNexus.setValues({ value: 3 })
        return []
      },
    })
    expect(() => rootNexus.setValues({ value: 2 })).toThrowError(
      expect.objectContaining({ code: 'reentrant-write' }),
    )
  })
})
