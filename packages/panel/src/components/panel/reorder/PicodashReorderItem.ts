...
import { usePicodashGroupContext } from '../../../state/groups/picodash-group-context.js'
import { usePicodashPanelStoreApi } from '../../../state/panel/picodash-panel-context.js'
import { keyboardReorderInteractionId } from '../../../state/order/picodash-order.js'
import { hasVisibleReorderableSibling } from '../../../state/order/picodash-order.js'
import type { PicodashPin } from '../../../state/panel/picodash-panel-types.js'
import { picodashMotionTokens } from '../../../lib/theme/theme.js'

export const reorderTransition: HTMLMotionProps<'div'>['transition'] = {
  layout: picodashMotionTokens.reorder,
  x: picodashMotionTokens.reorder,
  y: picodashMotionTokens.reorder,
}

export const reorderDragTransition: HTMLMotionProps<'div'>['dragTransition'] =
  picodashMotionTokens.reorderDrag
...
export function usePicodashReorderItem(
  itemId: string,
  ...
  const siblingOffsetY = useSpring(0, picodashMotionTokens.reorder)
  ...
  const commitReorder = () => {
    ...
    const animation = animate(visualDragOffsetY, 0, picodashMotionTokens.reorder)
