...
export type PicodashOrderBand = 'auto' | PicodashPin

export function partitionPicodashChildrenByBand(
  children: ReactNode,
  bandById: Readonly<Record<string, PicodashOrderBand>>,
) {
  const partition: Record<PicodashOrderBand, ReactNode[]> = {
    auto: [],
    end: [],
    start: [],
  }
  ...
  for (const child of Children.toArray(children)) {
    const itemId = itemIdFromPicodashChild(child)
    const band = itemId ? (bandById[itemId] ?? 'auto') : 'auto'
    partition[band].push(keyedPicodashChild(child, itemId))
  }
...
export function useOrderedPicodashChildren(
  ...
  return useMemo(() => orderPicodashChildren(children, orderedIds), [children, orderedIds])
}

export function orderPicodashChildren(children: ReactNode, orderedIds: string[]) {
  ...
  const entries = childArray.map((child) => {
    const itemId = itemIdFromPicodashChild(child)
    return { child: keyedPicodashChild(child, itemId), order: orderById.get(itemId ?? '') }
  })
...
function itemIdFromPicodashChild(child: ReactNode) {
  if (!isValidElement<PicodashChildProps>(child)) return undefined
...
function keyedPicodashChild(child: ReactNode, itemId: string | undefined) {
