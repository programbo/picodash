...
export interface PicodashVector3Props extends Omit<
  PicodashInputItemProps<PicodashVector3Value>,
  'children' | 'defaultValue' | 'parse'
> {
  defaultValue?: PicodashVector3Value
  max?: ReactiveProp<number>
  min?: ReactiveProp<number>
  step?: ReactiveProp<number>
}

const zeroVector: PicodashVector3Value = { x: 0, y: 0, z: 0 }
const axes = ['x', 'y', 'z'] as const

export function PicodashVector3({
  ...
  return (
    ...
    >
      {(control) => {
        ...
        return (
          <div className="col-span-2 grid min-w-0 grid-cols-3 gap-(--picodash-space-1)">
            {axes.map((axis) => (
              <VectorAxisInput
                key={axis}
                axis={axis}
...
function VectorAxisInput({
  ...
  control: PicodashItemContextValue<PicodashVector3Value>
  max: number | undefined
  min: number | undefined
  onValueChange: (value: number) => void
  step: number
  value: PicodashVector3Value
}) {
  ...
  return (
    <label className="relative min-w-0" htmlFor={inputId}>
      <span className="text-picodash-muted pointer-events-none absolute bottom-2 left-0 z-(--picodash-layer-raised) text-[0.5rem] leading-(--picodash-line-none) font-(--picodash-font-semibold) uppercase">
        {axis}
      </span>
      <Input
        id={inputId}
        aria-label={`${axis.toUpperCase()} axis`}
        className="h-(--picodash-control-height-sm) min-w-0 pr-0 pl-2 text-center tabular-nums"
...
export function normalizeVector3Value(
  value: unknown,
  fallback: PicodashVector3Value = zeroVector,
  min?: number,
  max?: number,
...
function isVector3Record(value: unknown): value is Partial<PicodashVector3Value> {
