...
export interface PicodashGradientProps extends Omit<
  PicodashInputItemProps<PicodashGradientValue>,
...
export function PicodashGradient({
  contentLayout = 'block',
  defaultValue,
  defaultRotation = 0,
  gradientClassName,
...
function GradientEditor({
  ...
  className?: string
  control: PicodashItemContextValue<PicodashGradientValue>
  fallbackValue: PicodashGradientValue
  ...
  const stops = normalizePicodashGradient(control.value ?? fallbackValue)
  ...
  const setStops = (nextStops: PicodashGradientValue) => {
    control.setInput(normalizePicodashGradient(nextStops))
  }
  ...
  const moveStopFromPointer = (event: ReactPointerEvent<HTMLButtonElement>, stopId: string) => {
    ...
    if (!rect) return
    const position = projectPicodashGradientPosition(event.clientX, rect)
  ...
  const gradientPreview = (
    <div
      ref={trackRef}
      className="border-picodash-control rounded-picodash-control relative h-(--picodash-control-height-lg) border shadow-(--picodash-shadow-inner)"
      id={control.inputId}
      style={{ backgroundImage: gradientCssValue(stops) }}
      onDoubleClick={(event) => {
        if (unavailable) return
        const rect = event.currentTarget.getBoundingClientRect()
        addStop(projectPicodashGradientPosition(event.clientX, rect))
      }}
    >
      {stops.map((stop) => (
        <button
  ...
  const stopEditor = (
    <div className="grid min-w-0 content-start gap-(--picodash-space-2)">
      <div className="grid grid-cols-[auto_minmax(0,1fr)_auto_auto] items-center gap-(--picodash-space-1-5)">
        <input
          aria-label="Selected stop color"
          className="border-picodash-control rounded-picodash-control size-(--picodash-control-height-sm) cursor-pointer border bg-transparent p-(--picodash-space-0-5) disabled:cursor-not-allowed disabled:opacity-(--picodash-opacity-disabled)"
          disabled={unavailable || !selectedStop}
          type="color"
          value={selectedStop?.color ?? '#000000'}
          onChange={(event) => {
            if (!selectedStop) return
            const color = normalizePicodashHexColor(event.currentTarget.value)
            setStops(stops.map((stop) => (stop.id === selectedStop.id ? { ...stop, color } : stop)))
          }}
        />
        <Input
          aria-label="Selected stop position"
          className="h-(--picodash-control-height-sm) min-w-0"
        ...
        <Button
          ...
          className="size-(--picodash-control-height-sm)"
          disabled={unavailable}
          size="icon"
          variant="outline"
          onClick={() => addStop(selectedStop?.position ?? 0.5)}
        >
          <Plus className="size-(--picodash-icon-sm)" aria-hidden="true" />
        </Button>
        <Button
        ...
        >
          <Trash2 className="size-(--picodash-icon-sm)" aria-hidden="true" />
        </Button>
      </div>
    </div>
  )
  ...
  return (
    <div className={cn('col-span-full grid gap-(--picodash-space-5)', className)}>
      {gradientPreview}
      <div
        className={cn(
          'grid gap-(--picodash-space-3)',
  ...
  return (
    <div className="grid min-w-0 content-start">
      <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-start">
        <div className="relative h-(--picodash-icon-md) min-w-0">
          <div
            aria-hidden="true"
          ...
          >
            <span className="bg-picodash-text absolute inset-y-0 left-0 w-(--picodash-border-thin)" />
            <span className="bg-picodash-text absolute inset-y-0 right-0 w-(--picodash-border-thin)" />
          </div>
          <input
            aria-label="Gradient rotation"
            className="accent-picodash-accent [&::-moz-range-thumb]:border-picodash-accent [&::-webkit-slider-thumb]:border-picodash-accent absolute top-1/2 left-(--_picodash-slider-hit-offset) z-(--picodash-layer-raised) h-(--picodash-icon-md) w-(--_picodash-slider-hit-width) min-w-0 -translate-y-1/2 cursor-pointer appearance-none bg-transparent outline-none disabled:cursor-not-allowed disabled:opacity-(--picodash-opacity-disabled) [&::-moz-range-thumb]:size-(--_picodash-slider-thumb-size) [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:border [&::-moz-range-thumb]:bg-(--_picodash-slider-thumb) [&::-moz-range-thumb]:transition-transform [&::-moz-range-thumb]:duration-(--picodash-duration-fast) hover:[&::-moz-range-thumb]:scale-110 [&::-moz-range-track]:h-(--_picodash-slider-track-height) [&::-moz-range-track]:bg-transparent [&::-webkit-slider-runnable-track]:h-(--_picodash-slider-track-height) [&::-webkit-slider-runnable-track]:bg-transparent [&::-webkit-slider-thumb]:mt-(--_picodash-slider-webkit-thumb-offset) [&::-webkit-slider-thumb]:size-(--_picodash-slider-thumb-size) [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:border [&::-webkit-slider-thumb]:bg-(--_picodash-slider-thumb) [&::-webkit-slider-thumb]:transition-transform [&::-webkit-slider-thumb]:duration-(--picodash-duration-fast) hover:[&::-webkit-slider-thumb]:scale-110"
        ...
        <output
          className="text-picodash-text ml-(--picodash-space-2) min-w-[4.5ch] self-center text-right text-(length:--picodash-font-size-lg) leading-(--picodash-line-none) font-(--picodash-font-normal) tabular-nums"
...
function handleStopKeyDown(
  ...
  stop: PicodashGradientStop,
  stops: PicodashGradientValue,
  unavailable: boolean,
  setStops: (stops: PicodashGradientValue) => void,
...
export function normalizePicodashGradient(value: unknown): PicodashGradientValue {
  ...
  const stops = source.map((stop, index) => {
    ...
    usedIds.add(id)
    return {
      color: normalizePicodashHexColor(stop.color),
      id,
...
export function normalizePicodashHexColor(value: unknown) {
...
export function projectPicodashGradientPosition(
  clientX: number,
...
export function gradientCssValue(stops: PicodashGradientValue, rotation?: number) {
  const normalized = normalizePicodashGradient(stops)
  const direction =
...
function colorAtGradientPosition(stops: PicodashGradientValue, position: number) {
  const normalized = normalizePicodashGradient(stops)
