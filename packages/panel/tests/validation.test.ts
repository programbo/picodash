...
test('uses Standard Schema output and rejects asynchronous validators', () => {
  const store = createPicodashPanelStore({ panelId: 'validation' })
...
test('shared field parsers must agree on one canonical output', () => {
  const store = createPicodashPanelStore({ panelId: 'validation' })
  const plusOne: PicodashParser = (input) => ({
    output: { value: Number(input) + 1 },
    success: true,
  })
  const plusTwo: PicodashParser = (input) => ({
...
test('retargeting reconciles both fields in one transaction and leaves an acceptable repair', () => {
  ...
  const boundedParser: PicodashParser = (input) =>
...
test('malformed parser results return field errors without throwing or mutating accepted values', () => {
  ...
  for (const [index, invalidResult] of invalidResults.entries()) {
    ...
    const parse = (() => invalidResult) as unknown as PicodashParser
...
test('import analysis reports repairs and applies the reviewed document atomically', () => {
  ...
  const analysis = analyzePicodashPanelDocument({ count: 2.6, title: 'Imported' }, store.getState())
