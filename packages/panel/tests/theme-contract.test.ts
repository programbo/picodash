...
const tokenPattern = /--(?:_)?tweaker-[a-z0-9-]+/g
const declarationPattern = /(--(?:_)?tweaker-[a-z0-9-]+)\s*:/g
...
test('keeps the public theme contract declared, used, documented, and semantic', () => {
  ...
  const publicTokens = declarations.filter((token) => !token.startsWith('--_tweaker-'))
  ...
  expect(
    publicTokens.filter(
      (token) =>
        token !== '--tweaker-panel-width' &&
        token !== '--tweaker-field-surface-min-height' &&
        componentFamilies.some((family) => token.startsWith(`--tweaker-${family}-`)),
    ),
  ).toEqual([])
