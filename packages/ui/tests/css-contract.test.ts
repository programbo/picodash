...
describe('@picodash/ui stylesheet contract', () => {
  ...
  it('defines reachable interactive recipes for controls and overlays', async () => {
    const css = await readFile(stylesheetPath, 'utf8')
    expect(css).toContain("[data-slot='button'][data-hovered]")
    expect(css).toContain("[data-slot='button'][data-pressed]")
    expect(css).toContain("[data-slot='button'][data-focus-visible]")
    expect(css).toContain("[data-slot='button'][aria-expanded='true']")
    expect(css).toContain("[data-slot='button'][data-disabled]")
    expect(css).toContain("[data-slot='button'][data-pending]")
    expect(css).toContain("[data-slot='action-menu-item'][data-focused]")
    expect(css).toContain("[data-slot='action-menu-item'][data-hovered]")
    expect(css).toContain("[data-slot='action-menu-item'][data-pressed]")
    expect(css).toContain("[data-slot='action-submenu'][data-open]")
    expect(css).toContain("[data-slot='alert-dialog-overlay']")
    expect(css).toContain("[data-slot='alert-dialog-content']")
    expect(css).toContain("[data-slot='tooltip']")
    expect(css).toContain('backdrop-filter: blur(var(--picodash-blur-surface));')
    expect(css).toContain('outline: 2px solid var(--picodash-color-focus);')
