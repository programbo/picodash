...
| STORE-LIFE    | Destroy and use-after-destroy                     | [packages/store/tests/root-lifecycle.test.ts](../../packages/store/tests/root-lifecycle.test.ts), [packages/store/tests/integration.test.ts](../../packages/store/tests/integration.test.ts) | Partial   | root teardown, lease refusal, use-after-destroy, and detached survivors verified; adapter/persistence/diagnostics capabilities remain pending |
