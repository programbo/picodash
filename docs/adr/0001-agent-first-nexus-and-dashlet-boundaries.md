# ADR 0001: Agent-first Nexus and Dashlet boundaries

## Status

Superseded by [ADR 0002](0002-provider-level-nexus-and-scoped-views.md). This ADR remains as a
record of the prototype-era per-Panel Nexus model.
...
`@picodash/nexus/react` provides synchronous whole-record adapter wiring for host frameworks and
...
- Panels are represented by `PicodashPanel` and receive typed Nexus instances.
- Dashlets bind to typed field handles rather than string identifiers.
- Field handles are owned by the Nexus, so they are stable and type-safe across composition.
...
- Panel/Nexus state owns values, contracts, interaction state, drafts, ordering, and repair.
