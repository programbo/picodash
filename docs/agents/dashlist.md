...
[`../reference/dashlist.md`](../reference/dashlist.md), the Store agent instructions for field and
...
operated. Optimize for explicit JSX composition, typed Store bindings, local drafts, accessible
...
- Keep disabled and read-only policies separate from authorization and from reorder/disclosure
  behavior.
- Attach labels, descriptions, and Store issues to the controls that consume them. Attribute
...
- Field defaults and validation belong to Store. Ready-made Dashlets do not add alternate value
