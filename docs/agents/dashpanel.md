...
Use Dev Bridge while developing Store-backed layout behavior whenever disclosed state,
...
- Persist settled layout overrides through Store. Do not persist visibility, activation, z-order,
...
- Keep the public high-level runtime API narrow. Do not add a launcher registry, mutable Provider
  store, generic selector, private controller channel, or Motion-specific public API.
