...
Phone-sized animation, boundary, and retained-theme evidence is generated by the admitted Contract
Lab tests in `apps/lab/tests/phone-artifacts.spec.ts`. Use `bun run artifacts:phone` for stable PNG,
WebM, and JSON copies; do not add another Playwright configuration or artifact-only route.
