...
Read this file before changing `@picodash/dev-bridge`, its Lab host, or agent-facing Store
inspection. Also read [`../development/agent-dev-bridge.md`](../development/agent-dev-bridge.md),
[`../../packages/dev-bridge/README.md`](../../packages/dev-bridge/README.md), and the Store agent
instructions when changing Store-facing behavior.

## Decision lens

Dev Bridge shortens the feedback loop between an agent's intent, public Store behavior, and the
...
improvement against a real browser Store consumer and feed general Store gaps back into Store.
...
- Never bypass Store validation or expose arbitrary evaluation, arbitrary Store access, filesystem
