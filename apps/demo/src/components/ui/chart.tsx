...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const [item] = payload
    const key = chartPayloadKey(labelKey ?? item?.dataKey ?? item?.name)
    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload
          ...
          .map((item, index) => {
            const key = chartPayloadKey(nameKey ?? item.name ?? item.dataKey)
            const itemConfig = getPayloadConfigFromPayload(config, item, key)
...
function chartPayloadKey(value: unknown) {
